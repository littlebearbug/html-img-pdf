...
- **🎨 高度可配置**: 支持背景色、图片质量、格式（PNG/JPEG）及自定义回调。
...
| `pageSize`        | `'auto'` \| `'a4'` \| `'letter'` ... | `'auto'`         | **'auto'**: 页面尺寸等于截图尺寸。<br>**'a4'等**: 使用标准纸张尺寸，内容按宽度缩放。<br>**{width, height}**: 自定义 PDF 页面宽高。 |
| `multipage`       | `boolean`                            | `false`          | 是否开启多页模式。开启后，`element` 的每个直接子元素将生成一页 PDF。                                                               |
| `imageFormat`     | `'png'` \| `'jpeg'`                  | `'png'`          | 截图生成的图片格式。JPEG 体积更小，PNG 支持透明。                                                                                  |
| `quality`         | `number`                             | `1.0`            | 图片质量 (0 - 1)，仅对 JPEG 有效。                                                                                                 |
| `backgroundColor` | `string`                             | `"#ffffff"`      | PDF 页面的背景颜色。                                                                                                               |
| `onClone`         | `(el: HTMLElement) => void`          | `undefined`      | 这是一个非常有用的回调。在截图前，你可以修改克隆出来的 DOM（例如隐藏按钮、修改文字颜色），而不影响页面上的真实元素。               |
